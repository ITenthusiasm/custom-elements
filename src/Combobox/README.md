...
- **Highly Customizable**: The `combobox` component is flexible enough to work with whatever CSS you provide, and its functionality can be enhanced or overriden through [class extension](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/extends).
- **Performant**: Unlike many other alternatives, the `combobox` component has been cleverly designed to work without complex state management tools or aggressively recurring DOM Tree manipulation. This makes it a fast and memory-efficient solution.
...
To use the component's built-in styles, you can use the `Combobox.css` file. If you're using a bundler like Vite, you can just import this directly into one of your JS files.
...
Remember that in `Select Enhancing Mode`, all relevant attributes will be copied from the `<select>` element to the relevant `<combobox-field>`/`<combobox-option>` elements on mount / page load.

### Configuring the Allowed Values

When the `<combobox-field>` is in `filter` mode, its allowed values can be configured with the [`valueis`](https://github.com/ITenthusiasm/custom-elements/tree/main/src/Combobox/docs/combobox-field.md#valueis) attribute. (This attribute does nothing if the component is not in `filter` mode.) There are 3 allowed values:

- `unclearable`: The `combobox` can only be given a value that matches one of its `option`s. If the user starts filtering the `option`s and exits the `combobox` without selecting an `option`, then the searchbox's text will be reset to the label of the currently-selected `option`.
...
There are some minor nuances that come with restyling the component. You can read about them in our [documentation](https://github.com/ITenthusiasm/custom-elements/tree/main/src/Combobox/docs/guides/README.md#custom-styling).
